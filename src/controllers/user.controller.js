...
const registerUser = asyncHandler(async (req, res) => {
  /*
  i)Get the user data from frontend
  ii)validation
  ii)check if user already exists: username, email
  iv)check for images, check for avatar
   v)upload them to cloudinary, avatar
   vi)create user object -> create entry in db
  vii)remove password and refresh token field from repsonse
  ...
  */
  ...
  //check if user already exists with the username or email
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if (newPassword !== confirmPassword) {
    throw new ApiError(400, "New Password and Comfirm Password do not match");
  }
  ...
  const isSameAsOldPassword = await user.isPasswordCorrect(newPassword);
